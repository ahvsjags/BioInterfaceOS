"""Tests for the strict T129 CC0 target-admission screening workflow."""
...
def test_audit_records_candidates_without_promoting_a_target(tmp_path: Path) -> None:
    workflow = CC0TargetAdmissionWorkflow(ROOT, output_root=tmp_path / "admission")

    summary = workflow.run(strict=True)
    receipt = workflow.verify()

    assert summary.candidate_source_count == 2
    assert summary.candidate_laboratory_count == 2
    assert summary.source_condition_count == 9
    assert receipt["status"] == "BLOCKED_NO_CC0_COMMON_TARGET"
...
def test_audit_requires_strict_mode(tmp_path: Path) -> None:
    ...
    with pytest.raises(CC0TargetAdmissionError, match="requires --strict"):
        workflow.run()
...
def test_audit_rejects_silent_candidate_promotion(tmp_path: Path) -> None:
    ...
    workflow = CC0TargetAdmissionWorkflow(
        ROOT,
        registry_path=registry_path,
        output_root=tmp_path / "admission",
    )

    with pytest.raises(CC0TargetAdmissionError, match="silently promotes a target"):
...
def test_audit_rejects_tampered_receipt(tmp_path: Path) -> None:
    workflow = CC0TargetAdmissionWorkflow(ROOT, output_root=tmp_path / "admission")
    workflow.run(strict=True)
    decision = tmp_path / "admission" / "target_admission_decision.json"
    decision.chmod(0o600)
    decision.write_text('{"model_fitted":true}\n', encoding="utf-8")

    with pytest.raises(CC0TargetAdmissionError, match="receipt is invalid"):
        workflow.verify()
