...
def test_t197_receipt_verifies_and_reexecutes_selection_in_null() -> None:
    ...
    assert all(item["development_only_target_count"] >= summary.target_count_minimum for item in report["fold_targets"])


def test_t197_fold_target_sets_are_not_the_all_source_intersection() -> None:
    report_path = ROOT / R4T197SourceAvailabilityWorkflow.OUTPUT_RELATIVE / "outer_fold_target_sets.json"
