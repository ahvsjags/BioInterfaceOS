...
def test_t200_t197_intervals_are_batch_clustered_and_t198_is_stratified() -> None:
    ...
    intervals = list(csv.DictReader((root / "t197_fold_metric_cluster_intervals.csv").open(encoding="utf-8", newline="")))
    ...
    assert {row["dimension"] for row in strata} == {"biological_unit_id", "clinical_group", "particle"}
