...
def test_manchester_protocol_freezes_analysis_only_accounting() -> None:
    ...
    assert expected["expected_external_observation_count"] == 4150
    assert expected["expected_measurement_batch_count"] == 288
    assert expected["expected_biological_unit_count"] == 60
...
def test_manchester_receipts_verify_when_analysis_artifacts_are_present() -> None:
    ...
    assert audit.source_cell_count == 193360
    assert audit.positive_source_cell_count == 177067
    assert audit.biological_unit_count == 60
    assert audit.measurement_batch_count == 288
    assert audit.shared_canonical_protein_count == 25
    assert ood.external_observation_count == 4150
    assert ood.external_measurement_batch_count == 288
    assert ood.biological_unit_count == 60
