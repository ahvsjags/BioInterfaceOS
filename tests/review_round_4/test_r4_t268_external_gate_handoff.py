...
def test_t268_binds_current_release_and_preserves_gate_boundary() -> None:
    ...
    release = _json("docs/release/R10_50_DOI_DEPOSIT_METADATA.json")

    assert protocol["fixed_release"]["tag"] == "v0.1.3-r10.50"
    assert protocol["fixed_release"]["source_commit"] == "07db8ceef9b785bc3fba0f79f346f9f633645a63"
    ...
    assert release["release"]["tag"] == "v0.1.3-r10.50"
    assert release["release"]["manifest_sha256"]
    assert release["release"]["archive_sha256"]
...
def test_t268_external_packages_and_helper_are_present() -> None:
    ...
    helper = ROOT / "scripts/r4_external_reproduction_r10_50.sh"

    assert lockbox["fixed_release"]["tag"] == "v0.1.3-r10.50"
    assert lockbox["non_author_required"] is True
    assert lockbox["current_gate_state"]["independent_validation"] is False
    assert adoption["fixed_release"]["tag"] == "v0.1.3-r10.50"
