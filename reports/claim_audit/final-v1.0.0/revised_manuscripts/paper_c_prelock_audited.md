# Development candidates for scientific interface laws: a pre-lock specification
...
T093 evaluated four symbolic candidates. Three candidates passed unit checks. One candidate was rejected for dimensional inconsistency. Nested study-CV used four outer folds. Expression bootstrap stability was 1.000000. The selected expression is `0.62*surface_norm + 0.28*functional_axis + 0.10*charge_norm`.
...
The pre-lock specification above is preserved as the frozen development record. The authorized evaluator exposed metadata-only statuses; no protected raw values were read.

| Claim | Prediction | Audited status | Failure/boundary |
|---|---|---|---|
| C1 | P1 | POSTLOCK_REPLICATED | none |
| C2 | P2 | POSTLOCK_REPLICATED | none |
| C3 | P3 | POSTLOCK_INCONCLUSIVE | protocol_boundary |
| C4 | P4 | POSTLOCK_REFUTED | overlap_failure |
| C5 | P5 | POSTLOCK_INCONCLUSIVE | model_disagreement |
| C6 | — | LANGUAGE_GATE_PRESERVED | preserved_boundary |
| C7 | — | APPLICABILITY_LIMIT_PRESERVED | preserved_boundary |
| C8 | — | EVALUATOR_AUTHORIZED_METADATA_ONLY | preserved_boundary |

C6 remains association-only; C7 retains OOD/selection limits; all abstentions remain visible.
