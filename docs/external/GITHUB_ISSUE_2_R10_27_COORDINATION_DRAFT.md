...
1. One non-author evaluator holding a protected held-out source or unseen real dataset. Submit aggregate results only; do not upload protected rows.
...
Interested participants may state which role they can perform and provide a public or private contact route. Do not attach protected data to the issue.

## Posting checklist

- Confirm the release link resolves to v0.1.3-r10.26.
- Preserve the existing issue history; replace only stale r10.24 text and do not delete prior comments.
- Keep the hard-gate state false until a submitted receipt passes structural preflight and independent identity/scope audit.
- Record any response as coordination only until the corresponding signed receipt and immutable archive locator exist.
