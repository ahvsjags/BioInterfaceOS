# Independent reproduction and external-user handoff for v0.1.3-r10.26
...
Use the immutable tag `v0.1.3-r10.26`. Resolve `git rev-parse 'v0.1.3-r10.26^{}'`, read `source_commit` and the manifest hash from `release/empirical_candidate_v0.1.3-r10.26/release_manifest.json`, and record all three values in the receipt. Do not use a moving branch for an external claim.

```bash
git clone --branch v0.1.3-r10.26 --depth 1 https://github.com/ahvsjags/BioInterfaceOS.git
cd BioInterfaceOS
git rev-parse 'v0.1.3-r10.26^{}'
sha256sum release/empirical_candidate_v0.1.3-r10.26/release_manifest.json
