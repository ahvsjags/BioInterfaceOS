# Independent reproduction and external-user handoff for v0.1.3-r10.24
...
Use the immutable tag `v0.1.3-r10.24` and the exact target commit in the release manifest. Do not use a moving branch for an external claim.

```bash
git clone --branch v0.1.3-r10.24 --depth 1 https://github.com/ahvsjags/BioInterfaceOS.git
cd BioInterfaceOS
uv sync --locked --all-groups
uv run pytest tests/review_round_3 tests/review_round_4 -q
```

T217 remains the paper-data statistical boundary: T195 is the primary nine-target leave-one-source-anchor-out exploratory route; T197 is same-lineage availability sensitivity; T198 is threshold/missingness sensitivity; T203/T209 are author-run paper-data OOD. Batch counts and paper-reported units are not cross-study biological n.
