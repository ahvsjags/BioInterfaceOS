...
| 角色 | 分数 | 编辑判断 |
|---|---:|---|
| 统计主编 | 79 | estimand、study-held-out、nested selection、cluster uncertainty 和 missingness audit 已成体系；MNAR/interaction 仍需正式闭合 |
...
| 数据审计员 | 82 | 三实验室/99 target 的行级 provenance 已核验，141-subject cohort 已完成 cell-level audit；biological lineage 边界仍需保守叙述 |
...
| 统计分析设计 | **89** | T217/T200/T195/T181 已固定 primary role、study-held-out、nested、cluster、missingness 和 multiplicity；还差正式 MNAR bounds/IPW 与 source-by-model interaction |
...
| 来源异质性与 claim discipline | **91** | 正向、负向和 near-zero 路线均保留，未做不当 pooled inference；仍需全文持续执行同一 claim audit |
...
| **强 Q1 综合成熟度** | **68** | 外部硬门禁具有不可替代性，不能由内部 90+ 模块抵消 |
...
5. 补齐 MNAR/pattern-mixture/IPW 或可识别 bounds，以及正式 source-by-model interaction，并在同一 immutable release 上复跑。
