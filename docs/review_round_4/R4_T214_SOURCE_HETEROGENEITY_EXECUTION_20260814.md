...
### 主 effect-unit table（不是五项独立研究）

主表保留 5 个不重复的 route-specific 描述性 effect units；这不是五项独立研究，也不是五个独立生物队列：
...
| T203 PMC10257194 | paper-attached processed cohort；author-run OOD；receipt 仅暴露 measurement-batch count，未将其当作 biological n | +0.0241 | [0.0202, 0.0284] | analysis-only exploratory |
...
- T198 八个 coverage thresholds 均保留，full-minus-composition 范围为 `+0.024199` 到 `+0.033323`；这些是 secondary batch-level missingness sensitivity，不是 T200 的 subject-equal primary estimand，也不是 confirmatory p-value family。
