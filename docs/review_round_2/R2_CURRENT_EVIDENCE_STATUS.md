...
|---|---|---|---|
| R2-01 | OPEN_EMPIRICAL_TARGET_UNAVAILABLE | 当前 T129 汇总覆盖 6 个候选来源、5 个实验室和 16 个已核验源资产。PXD030327 的 636 个来源 run 与 819 个矩阵列已映射，但 NP 标签仍为类别、数值字段仅为 P/NP 暴露且仅单实验室。T131 另验证 PXD017052 的 4 个 CC-BY 资产、9 个结果到原始文件匹配和 3 个 SPION 数值记录，但缺少原始文件到 SPION 身份的显式交叉表。未冻结共同 target，禁止模型使用 | 真实、非 fixture 的跨研究共同终点、数值材料/粒径协变量与行级溯源；冻结模型输出和外部 held-out 评估 |
...
| R2-09 | OPEN_EXTERNAL_ACCEPTANCE_REQUIRED | 项目仍为 IN_PROGRESS；T128 v1.3 绑定当前 T129/T131 证据与两条 protocol 稿件路径，现有九个阻塞项 | 外部复现、编辑复审、逐项 finding 映射与签名证明 |
