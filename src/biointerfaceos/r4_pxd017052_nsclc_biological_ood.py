...
class R4PXD017052NSCLCBOODWorkflow:
    ...
    def _protocol(self) -> tuple[dict[str, Any], dict[str, Path]]:
        ...
        if (
            ...
            or protocol.get("protocol_id") != "bioif-r4-pxd017052-nsclc-biological-ood-protocol-v1.0.0"
        ...
        if (
            ...
            or self.source_assets_root != (self.root / "data/raw/r4_candidate_pxd017052_nsclc").resolve(strict=False)
        ):
        ...
        required_external = {
            ...
            "biological_unit": "individual subject plasma sample identified by the first header token",
            "measurement_role": "NP_CORONA only; depleted plasma columns excluded",
            "expected_biological_unit_count": 141,
            "expected_measurement_batch_count": 705,
            "expected_rank_qualified_measurement_batch_count": 666,
            "expected_external_observation_count": 17026,
            "expected_shared_canonical_protein_count": 34,
            "minimum_proteins_per_measurement_batch": 10,
            "access_condition": "paper-attached CC-BY-4.0 Supplementary Data 5; author-run exploratory cohort OOD; not a protected lockbox and not an independent evaluator",  # noqa: E501
        }
    ...
    def _external_observations(
    ...
    ) -> tuple[list[_Observation], list[dict[str, Any]], set[str], dict[str, str]]:
        ...
        positive_by_batch: defaultdict[str, int] = defaultdict(int)
        for row in rows:
            if row["rank_target_eligible"] == "true":
                positive_by_batch[row["measurement_batch_id"]] += 1
        qualified = {
            batch
            for batch, count in positive_by_batch.items()
            if count >= external["minimum_proteins_per_measurement_batch"]
        }
    ...
    def _cluster_bootstrap(
        ...
        *,
        ...
        for unit in unit_rows:
            value = unit_rows[unit][metric]
            if value is not None:
                numeric_values.append(float(value))
        if len(numeric_values) != len(unit_rows):
            return None
    ...
    def run(self, *, strict: bool = False) -> R4PXD017052NSCLCBOODSummary:
        ...
        external, target_rows, accessions, batch_to_unit = self._external_observations(
            source_map, feature_values, protocol
        )
        ...
        for model_index, model_id in enumerate(helper.MODEL_IDS, start=1):
            ...
            spearman_interval = self._cluster_bootstrap(
                by_unit,
                "spearman",
                resamples=uncertainty["resamples"],
                seed=uncertainty["random_seed"] + model_index * 100,
            )
            mae_interval = self._cluster_bootstrap(
                by_unit,
                "mae",
                resamples=uncertainty["resamples"],
                seed=uncertainty["random_seed"] + model_index * 100 + 1,
            )
            rmse_interval = self._cluster_bootstrap(
                by_unit,
                "rmse",
                resamples=uncertainty["resamples"],
                seed=uncertainty["random_seed"] + model_index * 100 + 2,
            )
            ...
            if biological_unit_count is None:
                raise R4PXD017052NSCLCBOODError("biological unit count is undefined")
            model_rows.append(
                {
                    "model_id": model_id,
                    ...
                    **aggregate,
                    ...
                    if spearman_interval is None
                    else spearman_interval["lower_95"],
                    "subject_equal_mean_spearman_upper_95": None
                    if spearman_interval is None
                    else spearman_interval["upper_95"],
                    "subject_equal_mean_mae_lower_95": None if mae_interval is None else mae_interval["lower_95"],
                    "subject_equal_mean_mae_upper_95": None if mae_interval is None else mae_interval["upper_95"],
                    "subject_equal_mean_rmse_lower_95": None if rmse_interval is None else rmse_interval["lower_95"],
        ...
        paired_interval = self._cluster_bootstrap(
            paired_unit_values,
            "delta",
            resamples=uncertainty["resamples"],
        ...
        negative_summary = {
            ...
            "null_subject_equal_mean_spearman_mean": float(np.mean(null_primary)),
            "null_subject_equal_mean_spearman_lower_95": float(np.quantile(null_primary, 0.025)),
            "null_subject_equal_mean_spearman_upper_95": float(np.quantile(null_primary, 0.975)),
            "one_sided_upper_tail_p": float(
                (1 + sum(value >= observed_primary for value in null_primary)) / (1 + len(null_primary))
            ),
        }
        selection_rows = [
        ...
        report = {
            ...
            "paired_composition_ablation": {
                "paired_measurement_batch_count": len(paired_batches),
                "paired_biological_unit_count": len(paired_unit_values),
                "full_minus_composition_subject_equal_mean_spearman": paired_delta,
                "cluster_bootstrap": paired_interval,
            },
        ...
        return R4PXD017052NSCLCBOODSummary(
            len(development),
            len(external),
            len(accessions),
    ...
    def verify(self) -> R4PXD017052NSCLCBOODSummary:
        ...
        for value in artifacts.values():
            ...
            if set(item) != self.REQUIRED_REFERENCE or _sha256(
                self._root_file(_string(item.get("relative_path"), "artifact path"), "artifact")
            ) != _checksum(item.get("sha256"), "artifact"):
