...
class R4PMC10257194PaperOODWorkflow(R4SmallMoleculeCoronaOODWorkflow):
    ...
    SOURCE_AUDIT_WORKFLOW = R4PMC10257194PaperSourceAuditWorkflow
    SOURCE_AUDIT_ERROR = R4PMC10257194PaperSourceAuditError
    ...
    def _protocol(self) -> tuple[dict[str, Any], dict[str, Path]]:
        ...
        if protocol.get("protocol_id") != "bioif-r4-pmc10257194-paper-ood-protocol-v1.0.0" or protocol.get("evidence_class") != "DEVELOPMENT_OBSERVATION" or protocol.get("allowed_claim_level") != "EXPLORATORY":
        ...
        expected_refs = {"r3_analysis_protocol_receipt", "r3_common_target_ledger", "r3_sequence_feature_table", "r4_source_audit_receipt", "r4_source_cell_map"}
        ...
        if self.output_data_root != (self.root / "data/raw").resolve(strict=False) or self.feature_root != (self.root / "data/raw/r3_uniprot_sequence_features").resolve(strict=False) or self.source_assets_root != (self.root / "data/raw/r4_candidate_pmc10257194").resolve(strict=False):
        ...
        if external != {
            ...
            "analysis_population": "45 paper-attached NaY-PPC subject columns with exact frozen R3 target accession mapping and strictly positive finite values",
            ...
            "access_condition": "CC-BY-NC-ND-4.0 paper supplementary workbook; analysis-only local acquisition; not redistributable and not a protected lockbox",
        }:
            raise R4PMC10257194PaperOODError("PMC10257194 external evaluation contract is invalid")
        return protocol, paths

    def _external_observations(self, source_map_path: Path, feature_values: Mapping[str, tuple[float, ...]], protocol: Mapping[str, Any]):
        observations, target_rows, accessions = super()._external_observations(source_map_path, feature_values, protocol)
        external = protocol["external_evaluation"]
        if len({row.measurement_batch_id for row in observations}) != external["expected_measurement_batch_count"] or len({row.canonical_accession for row in observations}) < external["expected_shared_canonical_protein_count_at_least"]:
