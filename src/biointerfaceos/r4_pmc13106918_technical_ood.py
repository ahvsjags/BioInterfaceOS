...
from biointerfaceos.r4_pmc13106918_source_audit import (
    ...
    R4PMC13106918SourceAuditSummary,
)
...
class R4PMC13106918TechnicalOODWorkflow(R4SmallMoleculeCoronaOODWorkflow):
    ...
    def _protocol(self) -> tuple[dict[str, Any], dict[str, Path]]:
        ...
        if protocol.get("protocol_id") != "bioif-r4-pmc13106918-technical-ood-protocol-v1.0.0" or protocol.get(
            "evidence_class"
        ) != "DEVELOPMENT_OBSERVATION" or protocol.get("allowed_claim_level") != "EXPLORATORY":
        ...
        if self.output_data_root != (self.root / "data/raw").resolve(strict=False) or self.feature_root != (
            self.root / "data/raw/r3_uniprot_sequence_features"
        ).resolve(strict=False) or self.source_assets_root != (
            self.root / "data/raw/r4_candidate_pmc13106918"
        ...
        if external != {
            ...
            "access_condition": "public CC-BY-4.0 Zenodo package; author-run technical OOD candidate; one pooled material with technical replicates; not a protected lockbox and not an independent evaluator",
        }:
    ...
    def _external_observations(
        ...
        required = {
            ...
            "measurement_batch_id", "canonical_accession", "author_quantity_type", "author_numeric_value",
        ...
        eligible_batches = {
            batch for batch, values in by_batch.items() if len(values) >= external["minimum_proteins_per_measurement_batch"]
        }
        ...
        for row in rows:
            ...
            observations.append(
                _Observation(target_id, external["source_id"], accession, external["laboratory_anchor"], batch_id, percentile, feature_values[accession])
            )
            target_rows.append({
        ...
        if (
            ...
            or any(len(values) < external["minimum_proteins_per_measurement_batch"] for values in by_batch.values() if values[0]["measurement_batch_id"] in eligible_batches)
        ):
    ...
    def run(self, *, strict: bool = False) -> R4SmallMoleculeCoronaOODSummary:
        ...
        for model_index, model_id in enumerate(helper.MODEL_IDS, start=1):
            ...
            model_rows.append({
                "model_id": model_id, "external_observation_count": len(external), "external_measurement_batch_count": len(metrics), "primary_metric_status": status, **aggregate,
                "mean_spearman_lower_95": None if intervals["mean_spearman"] is None else intervals["mean_spearman"]["lower_95"], "mean_spearman_upper_95": None if intervals["mean_spearman"] is None else intervals["mean_spearman"]["upper_95"],
                "mean_mae_lower_95": intervals["mean_mae"]["lower_95"], "mean_mae_upper_95": intervals["mean_mae"]["upper_95"], "mean_rmse_lower_95": intervals["mean_rmse"]["lower_95"], "mean_rmse_upper_95": intervals["mean_rmse"]["upper_95"],
            })
        ...
        negative_summary = {"selected_alpha": full_alpha, "resamples": negative["resamples"], "random_seed": negative["random_seed"], "observed_mean_spearman": full_primary, "null_mean_spearman_mean": float(np.mean(null_primary)), "null_mean_spearman_lower_95": float(np.quantile(null_primary, 0.025)), "null_mean_spearman_upper_95": float(np.quantile(null_primary, 0.975)), "one_sided_upper_tail_p": float((1 + sum(value >= full_primary for value in null_primary)) / (1 + len(null_primary)))}
